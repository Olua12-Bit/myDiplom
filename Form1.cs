    ...
    private void GetImage(string sim,string p_name,ListViewItem item)
        {
            ...
            try
            {
                ...
                if (ctg != null)
                {
                    ...
                    myImgList.ImageSize = new Size(190, 220);
                    myImgList.ColorDepth = ColorDepth.Depth32Bit;
        ...
        private void DelCtg()
        {
            try
            {
                ...
                string sql = "DELETE FROM Picture WHERE ct_id=" + ctg + "";
                cmd = new OleDbCommand(sql, conn);
                cmd.ExecuteNonQuery();
                ...
                {
                    sql = "DELETE FROM Category WHERE ct_id=" + ctg + "";
                    cmd = new OleDbCommand(sql, conn);
                    cmd.ExecuteNonQuery();
                }
        ...
        private void AddCtg()
        {
            ...
            if (textBox1.Text == "" || textBox1.Text == " " || textBox1.Text == null)
                {
                    MessageBox.Show("Вы не ввели наименование категории");
                }
                else
                {
        ...
        private void AddImage(string im_name, string im_path, string ct_id)
        {
            try
            {
                conn = DBUtils.GetDBConnection();
                ...
                {
                    MessageBox.Show("Выберете категорию в списке");
                }
            }
            catch (Exception ex) {
                MessageBox.Show(ex.ToString());
        ...
        {
            if(Ctg_cmBox.SelectedItem!=null)
            {
                DelCtg();
            }
            else
        ...
        private void button1_Click_2(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Выберите категорию в списке");
            }
            else if (Ctg_cmBox.SelectedIndex != -1 || Ctg_cmBox.SelectedIndex > 0)
            {
                ...
                if (ans == DialogResult.OK)
                {
                    ...
                    AddImage(getName + end, "files\\" + getName + end, ctg);
                }
